...
#### PC
class GradientRestriction(Function):
...
def Restriction_layer(x, _lambda):
    return GradientRestriction.apply(x, _lambda)
...
class GLIP(SingleStageDetector):
    ...
    def __init__(self,
                 ...
                 backbone_gr: ConfigType,
                 backbone_neck_gr: ConfigType,
                 _alpha: ConfigType,
        ...
        backbone_out_indices = backbone.out_indices
        emb_dim = backbone.embed_dims
            
        with_adapter = False
        
        self.with_backbone = backbone_gr
        self.with_backbone_neck = backbone_neck_gr
        
        self.with_adapter = with_adapter
        if with_adapter:
            self.adapters = [AdapterForward(emb_dim * 2**i, 60, emb_dim * 2**i).cuda() for i in backbone_out_indices]
    ...
    def loss(self, batch_inputs: Tensor,
        ...
        # befor neck
        only_backbone = self.with_backbone
        if only_backbone:
            visual_features = self.extract_feat_backbone(batch_inputs)

        # after neck
        backbone_neck = self.with_backbone_neck
        if backbone_neck:
            visual_features = self.extract_feat(batch_inputs)      
            _alpha = self._alpha        
            visual_features = tuple(Restriction_layer(feat, _alpha) for feat in visual_features)
    ...
    def extract_feat_backbone(self, batch_inputs: Tensor) -> Tuple[Tensor]:       
        x = self.backbone(batch_inputs)
        x = tuple(Restriction_layer(feat, self._alpha) for feat in x)
        if self.with_neck:
            x = self.neck(x)            
        return x 
    ...
    def predict(self,
        ...
        # txt =('Toad', 'Pipefish', 'StickInsect', 'Katydid', 'Cat', 'Octopus',
        #        'Leopard', 'Centipede', 'GhostPipefish', 'StarFish', 'Owl', 'Bird', 'Kangaroo',
        #        'Spider', 'Chameleon', 'Mockingbird', 'Grasshopper', 'Cheetah', 'Gecko',
        #        'Bug', 'Mantis', 'SeaHorse', 'Crab', 'ScorpionFish', 'Giraffe', 'Butterfly',
        #        'Cicada', 'Shrimp', 'Caterpillar', 'Dog', 'Fish', 'LeafySeaDragon', 'Dragonfly',
        #        'Lizard', 'Snake', 'Other', 'Bee', 'Worm', 'Ant', 'Human', 'Deer', 'Duck', 'Sciuridae',
        #        'BatFish', 'Frog', 'Bittern', 'ClownFish', 'Turtle', 'Flounder', 'Heron', 'Frogmouth',
        #        'Stingaree', 'Bat', 'Moth', 'Tiger', 'Grouse', 'Pagurian', 'Rabbit', 'Lion', 'Wolf', 'FrogFish',
        #        'Owlfly', 'Crocodile', 'Sheep', 'Monkey', 'Slug', 'Beetle', 'CrocodileFish', 'Reccoon')

        txt =('flatfish', 'octopus', 'owl', 'person', 'snake', 
               'insect', 'cat', 'spider', 'lizard', 'crab', 'squirrel', 
               'frog', 'fish', 'sheep', 'dragonfly', 'rabbit', 'turtle', 
               'shrimp', 'butterfly', 'bird', 'sea horse', 'sea dragon', 
               'crocodile', 'bear', 'starfish', 'leopard', 'deer', 'wolf', 
               'koala', 'tiger', 'giraffe', 'mouse', 'fox', 'lion', 'moth', 'dog', 'otter',
               )
        for data_samples in batch_data_samples:
            #text_prompts.append(data_samples.text)
            text_prompts.append(txt)
        ...
        for img_data, data_sample, pred_instances, entity in zip(batch_inputs, batch_data_samples,
                                                       results_list, entities):
            
            #self.pcb.execute_calibration(img_data, pred_instances)
